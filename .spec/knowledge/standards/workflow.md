...
- 公共架构变更不从本仓直接发起：跨语言可达面与状态码是架构仓 `engine/abi/native-abi.json` 的事，先在那边改；本仓不保存镜像，也不为内核类型申请状态码（ADR 0009）。
...
- 根 `.github/CODEOWNERS` 当前将全仓指派给 `@Go1c`；涉及公共 ABI、unsafe/FFI、线程、内存、平台或供应链的改动必须由对应所有者审查。
- 合并前必须通过 Repository Policy、全部适用的 Miri/Sanitizer/Benchmark 证据、收口门槛与 reviewer 闭环。
