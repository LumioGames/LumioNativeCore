...
- Kernel 实现使用 Rust，跨语言边界使用版本化 C ABI；FFI 类型必须明确布局、所有权、线程、取消和错误语义。
- 当前仓库尚未提交 Cargo 工程。首次引入代码时必须固定 Rust toolchain、`rustfmt`、`clippy`、Miri/Sanitizer/依赖审计与可复现验证命令，并更新本文和 [`testing.md`](./testing.md)。
- Rust 模块、文件、函数与局部变量使用 `snake_case`，类型与 trait 使用 `PascalCase`，常量使用 `SCREAMING_SNAKE_CASE`；已发布 ABI/Schema/Manifest 标识符保持原拼写。
- 规范正文使用中文，代码标识符、协议字段和命令保留原始英文；Markdown 与结构化文本保持 LF（见根 `.gitattributes`）。
- ABI Header、Binding、Capability/Error 元数据和 Manifest 等生成物只从锁定源生成，记录 Compiler/Input/Output Hash，不手改生成结果。
