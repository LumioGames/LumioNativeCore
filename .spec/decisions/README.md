...
> 跨仓公共语义的决策只在架构仓 `LumioGameEngine` 维护；本目录仅记录 NativeCore 内部实现决策，并从 `0001` 开始编号。
...
| 编号 | 决策 | 状态 |
|------|------|------|
| [0001](0001-contract-layering-and-symbol-surface.md) | abi 拆 contract-types 叶子与 native-core-ffi 门面,不导出跨仓 Root 符号 | 被 0009 取代 |
...
| [0006](0006-capability-keys-have-no-raw-constructor.md) | Capability key 只由生成注册表投影,不保留裸构造器 | 被 0009 取代 |
...
| [0009](0009-exit-legacy-contract-regime.md) | 退出旧合同制:删镜像与 CI 断言、删 native-ffi 与 contract-types、内核不再持有跨边界数值 | 生效 |
